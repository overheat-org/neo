...
test {
    const code = codegen(Node.new(.If, .{ .expect = Node.new(.Number, .{ .value = 5 }), .then = Node.new(.Null, .{}), .children = Node.new(.If, .{ .expect = Node.new(.Number, .{ .value = 5 }), .then = Node.new(.Null, .{}), .children = null }) }), null);
